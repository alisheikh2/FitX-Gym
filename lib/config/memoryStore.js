...
export function buildModel(store, name) {
  class Model {
    ...
    static async create(data) {
      ...
      // IMPORTANT: if caller provides an _id (e.g. seed defaults with fixed IDs),
      // respect it — DO NOT overwrite. This keeps JWT tokens valid across cold boots.
      const now = new Date();
      let join = data.joinDate || data.startDate || now;
      if (!(join instanceof Date)) join = new Date(join);
      if (isNaN(join.getTime())) join = now;
      const _id = data._id || uid();
      const d = { _id, createdAt: now, updatedAt: now, ...data, joinDate: join, startDate: join };
      if (d.expiryDate && !(d.expiryDate instanceof Date)) {
        const ed = new Date(d.expiryDate);
        if (!isNaN(ed.getTime())) d.expiryDate = ed;
      }
      store.push(d);
      return d;
    }
